...
function menuOpenSettings() {
  ...
  showReport_('발송 설정',
    [
      ...
      '  발신번호        사전등록한 학원 대표번호',
      '  문구            ⚠ 알림톡 사용 중에는 수정 금지 (수정하면 확인 창이 뜹니다)',
      '                  등·하원 공용입니다. #{구분} 이 출석/하원으로 갈립니다',
      '  발신프로필키    알림톡 발신프로필 키 (비우면 SMS 로만 발송)',
      '  템플릿ID        심사 통과한 알림톡 템플릿 코드 (한 벌만 쓰므로 하나)',
      '  SMS폴백사용     TRUE / FALSE',
      '',
      '  쓸 수 있는 변수: #{학원명} #{학생명} #{일자} #{시각} #{구분}',
      '  이모지는 알림톡에만 나갑니다. SMS 로 대체될 때는 자동으로 빠집니다.',
      '',
