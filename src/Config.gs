 ...
 * 알림 문구.
 *
 * 알리고에서 등원·하원을 각각 심사받았다. 그래서 문구도 두 벌이다.
 * 심사 통과한 글자와 하나라도 다르면 발송이 거부되므로, 여기 있는 값은
 * 콘솔에 올린 전문과 똑같아야 한다.
 *
 * #{학원명} 은 기본정보 시트의 학원명으로 바뀐다. 승인 문구의 학원명
 * 자리에 그대로 들어가므로 결과 글자는 같다.
 */
var MSG_DEFAULT_IN = [
  '[#{학원명}🎶] - 등원 알림',
  '',
  '#{날짜} #{학생명} 학생이 #{시간}에 등원하였습니다.✨',
  '어썸음악학원은 아이들에게 언제나 열려 있습니다. 우리 아이들의 하루에 음악이 즐겁게 머물 수 있도록, 따뜻한 마음으로 함께하겠습니다.'
].join('\n');

// 등원과 같은 짜임이고 '등원' 자리와 끝 이모지만 다르다.
// 그래도 두 벌을 각각 심사받았으므로 한 벌로 묶지 않는다.
var MSG_DEFAULT_OUT = [
  '[#{학원명}🎶] - 하원 알림',
  '',
  '#{날짜} #{학생명} 학생이 #{시간}에 하원하였습니다.🍀',
  '어썸음악학원은 아이들에게 언제나 열려 있습니다. 우리 아이들의 하루에 음악이 즐겁게 머물 수 있도록, 따뜻한 마음으로 함께하겠습니다.'
].join('\n');

/**
 * 알림톡이 막혔을 때 대신 갈 문자(대체발송).
 *
 * 알림톡 본문과 다르다. 문자는 EUC-KR 이라 이모지를 못 쓰고, 90바이트를
 * 넘으면 LMS 가 되어 단가가 3배다. 그래서 인사말을 뺀 짧은 문안으로
 * 따로 등록했다.
 *
 * 비워 두면 알림톡 문구에서 이모지만 걷어내 보낸다.
 */
var SMS_DEFAULT_IN = [
  '[#{학원명}] - 등원 알림',
  '',
 ...
 *
 * 지금 문구는 등원·하원이 따로라 쓰지 않지만, 한 벌로 합쳐 쓰고 싶을 때를
 * 위해 변수는 그대로 둔다.
 */
var KIND_WORD = { 등원: '등원', 하원: '하원' };
var KIND_EMOJI = { 등원: '✨', 하원: '🍀' };

/** 문구 칸에 붙일 경고. 네 칸이 같은 규칙을 따른다. */
var MSG_SETTING_NOTE =
...
var SETTING_DEFAULTS = {
  ...
  문구_등원: [MSG_DEFAULT_IN, MSG_SETTING_NOTE + ' (알림톡 등원 본문)'],
  문구_하원: [MSG_DEFAULT_OUT, MSG_SETTING_NOTE + ' (알림톡 하원 본문)'],
  문자문구_등원: [SMS_DEFAULT_IN,
    '알림톡이 막혔을 때 대신 갈 문자. 이모지를 쓸 수 없고 90바이트를 넘으면 LMS 가 됩니다. ' +
    '비우면 알림톡 문구에서 이모지만 빼고 보냅니다.'],
  문자문구_하원: [SMS_DEFAULT_OUT, '위 문자문구_등원과 같은 규칙입니다.'],
  발신번호: ['', '통신서비스 이용증명원으로 사전등록한 학원 대표번호'],
  공급사: ['', 'solapi 또는 aligo'],
  발신프로필키: ['', '알림톡 발신프로필 키 (알리고는 senderkey)'],
  템플릿ID_등원: ['', '심사 통과한 등원 템플릿 코드'],
  템플릿ID_하원: ['', '심사 통과한 하원 템플릿 코드'],
  SMS폴백사용: ['TRUE', '알림톡 실패 시 SMS로 대체 발송'],
  오입력방지: ['키오스크',
    '등원·하원을 누르면 화면의 두 자리 숫자를 따라 눌러야 기록됩니다. ' +
    '아이들이 화면을 마구 눌러 엉뚱한 학생이 출석 처리되는 것을 막습니다. ' +
    '키오스크(현관 태블릿만) / 항상(직원 태블릿도) / 끔 중에서 고릅니다. ' +
    '관리자 모드에서는 어느 값이든 뜨지 않습니다.'],
  발송사용: ['TRUE',
    '자동 발송 스위치. FALSE 면 학생이 눌러도 알림이 나가지 않고 ' +
    '미발송(발송끔) 으로 남습니다. 출결 기능을 시험하는 동안 요금을 쓰지 않으려는 용도입니다. ' +
    '메뉴의 [시험 발송] 은 사람이 직접 누르는 것이라 이 값과 무관하게 나갑니다.'],
  테스트수신번호: ['',
    '값이 있으면 모든 알림이 보호자 대신 이 번호로만 갑니다. ' +
    '개발·검증 중에 실제 보호자에게 나가는 것을 막는 용도입니다. ' +
    '검증이 끝나면 반드시 비워야 합니다. 비우지 않으면 보호자는 아무것도 받지 못합니다.'],
...
/**
 * 인증정보가 틀렸을 때 어디를 보라고 할지.
 *
 * 넣는 길이 둘(편집기 UI, npm run secrets)이라 한쪽만 짚으면
 * "나는 그거 안 했는데" 가 된다. 둘 다 적는다.
 */
/**
 * 시험 발송에 쓰는 이름.
 *
 * 실제 학생 이름을 쓰면, 시험 문자를 받은 사람이 그 아이에게
 * 무슨 일이 생긴 줄 안다. 없는 이름을 쓴다.
 */
var TEST_SEND_NAME = '남궁철수';

var SECRET_HELP = [
  '⚙ 프로젝트 설정 → 스크립트 속성 에서 값을 확인하세요.',
  '또는 npm run secrets 로 만든 줄을 함수로 감싸 실행하세요.'
];
